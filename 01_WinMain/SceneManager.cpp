...
void SceneManager::Update()
{
	if (mCurrentScene != nullptr) {
		if (mCurrentScene == mLoadingScene) {
			if (mCurrentScene->GetIsEndLoading()) { //로딩 스크린 종료
				mCurrentScene = mTargetScene; //씬 시작
				mCurrentScene->Init();
			}
		}
...
void SceneManager::LoadScene(const wstring & targetSceneName, const wstring & loadingSceneName) //쓰레드 방식 수정 필요
{
...
void SceneManager::LoadSceneLoading(const wstring& targetSceneName, const wstring& loadingSceneName) {  //No쓰레드 방식
	SceneIter iter = mSceneList.find(targetSceneName);
	if (iter != mSceneList.end())
		mTargetScene = iter->second;
